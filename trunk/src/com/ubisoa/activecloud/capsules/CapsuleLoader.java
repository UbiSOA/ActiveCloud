		...
		try{
			...
			if(o != null){
				...
				if(theHardwareClass.isInstance(o)){
					...
					log.debug("Getting the config element");
					
					Element config = root.getChild("config");
					if(config != null){
						log.debug("Config element not null, calling setConfigElement");
					}else{
						log.debug("Config element null, calling setConfigElement with empty key list");
						config = new Element("config");
					}
					((NotificationCapsule)o).setConfigElement(config);
